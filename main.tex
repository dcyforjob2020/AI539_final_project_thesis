...
We build the dataset from two public software engineering interview question sources: the \emph{Software Engineering Interview Questions} dataset \cite{haris2024software} and the \emph{ai\_interview\_questions} set. After preprocessing we obtain 4,653 training examples and a held-out test set of 200 examples. Because preference candidate generation, two-pass AI judging, SFT, and DPO training are all computationally expensive under our Colab/GPU-time constraints, we did not use the full training pool for the preference optimization stage. Instead, we randomly sampled 2,000 examples from the 4,653 training examples for preference candidate generation and DPO training. These 2,000 examples were re-indexed from \texttt{train\_0001} to \texttt{train\_2000}. The held-out 200-example test set was kept unchanged throughout the project and was reused for both baseline and final evaluation. Each example pairs an interview question with a reference answer, category and difficulty metadata, and a simulated student answer.
...
We employ Qwen3.5-9B as the judge. While we initially attempted to use a larger teacher model (Qwen3.6-35B-A3B) to obtain higher-quality preference labels, the model encountered persistent Out-Of-Memory (OOM) errors and instability in our Google Colab environment (A100 GPU, 40GB VRAM). Even after implementing memory-safe loading with 4-bit quantization, CPU offloading, and custom monkey-patches for tensor management, the 35B model's instability remained a bottleneck. We consequently transitioned to Qwen3.5-9B, which provided the necessary stability and throughput for labeling the training set while maintaining high reasoning performance. 

To ensure consistent and structural output, we implemented several prompt engineering refinements. We found that base judge models often outputted lengthy ``thinking'' monologues or conversational fillers that interfered with automated JSON parsing. We addressed this using \emph{assistant pre-filling}: by appending the starting character of a JSON object (\texttt{\{}) to the model's prompt, we successfully suppressed the monologue and forced the model to immediately begin generating the structured score data. 
...
For preference selection, the judge compares candidates using a two-pass swapped-order protocol to mitigate \emph{position bias}---the common tendency of LLMs to systematically favor the first or second response regardless of actual quality. If left unaddressed, this bias would introduce significant noise into the RLAIF pipeline, causing the model to optimize for arbitrary structural artifacts rather than genuine coaching merit. In the first pass, \texttt{feedback\_a} is presented in the first slot and \texttt{feedback\_b} in the second; in the second pass, the display order is inverted. The judge assigns independent 1--20 scores for each slot in both passes. We then calculate the final quality score for each candidate by averaging its score across both presentations (e.g., averaging the score of \texttt{feedback\_a} when it was in the first slot with its score when it was in the second). This averaging process ensures that the \textit{chosen} and \textit{rejected} labels used for DPO reflect consistent, position-independent quality, resulting in a significantly cleaner optimization signal.
